...
function Home() {
  ...
  const baznas_user = (localStorage.getItem('baznas_user'));
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        setIsLoading(true);
        const sanityData = await sanityClient.fetch(`*[_type == 'baznas-entry']{
  ...
  useEffect(() => {
    const initializeMap = async () => {
      const data = serverData && serverData.data && serverData.data.length > 0
        ? serverData.data.filter(item => item.user.name === baznas_user)
        : [];
  ...
  const downloadExcel = () => {
    ...
    XLSX.writeFile(workbook, `baznas-entry-${volunteerName}.xlsx`);
  };
      ...
      <div className="w-full right-0 px-10 py-3 cursor-pointer bg-white">
        <div className="flex lg:justify-between justify-center items-center">
          <div className="flex justify-center gap-4 items-center">
              <img className="rounded-full h-14" src="https://upload.wikimedia.org/wikipedia/commons/e/e6/Logo_BAZNAS_RI-Hijau-01.png" />
              <h3 className="text-gray-800 lg:text-xl font-semibold">GIS Peta Sebaran Pendayagunaan <span className="text-green-700">BAZNAS NTB</span></h3>
          </div>
        ...
        <div className="relative mx-auto max-w-screen-xl px-4 py-32 sm:px-6 lg:flex lg:h-screen lg:items-center lg:px-8">
          <div className="max-w-xl text-center sm:text-left">
            ...
            <p className="mt-4 max-w-lg sm:text-xl/relaxed text-gray-700">
              Daftar Rekap Data Penerima Bantuan BAZNAS NTB.
            </p>
