...
function Biodata() {
  ...
  const baznas_userData = JSON.parse(localStorage.getItem('baznas_userData'));
  const baznas_id = (localStorage.getItem('baznas_id'));
  console.log('cek id user: ', baznas_id)
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        ...
        // Filter the data array based on baznas_id
        const filteredData = sanityData.filter(item => item._id === baznas_id);
  ...
  const updateSanityUser = async (baznas_userData) => {
    try {
      const response = await fetch(`https://ln9ujpru.api.sanity.io/v2021-03-25/data/mutate/production`, {
        ...
        body: JSON.stringify({
          mutations: [
            {
              patch: {
                id: baznas_id, // The _id of the document to update
                set: {
                  umur: baznas_userData.umur,
                  gender: baznas_userData.gender,
                  alamat: baznas_userData.alamat,
                  faskes: baznas_userData.faskes,
                  tb: baznas_userData.tb,
                  bb: baznas_userData.bb,
                  keluhan: baznas_userData.keluhan,
                  telepon: baznas_userData.telepon,
                },
              },
            },
          ],
        ...
        <div className="lg:flex">
          ...
          <div className="lg:w-1/2 lg:py-0 py-10 flex items-center">
            <form className="w-full lg:px-28 px-4 items-center" onSubmit={handleSubmit}>
              <h2 className="font-bold text-[18px] lg:text-4xl mb-10 text-gray-900 uppercase">Biodata <span className="text-sm">({baznas_userData[0]?.name})</span></h2>
              <Input
                type="text"
                name="umur"
                placeholder="Umur"
