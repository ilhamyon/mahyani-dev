...
function Login() {
  ...
  useEffect(() => {
    async function fetchSanityData() {
      try {
        const sanityData = await sanityClient.fetch(`*[_type == 'user-baznas']{
          _id,
          name,
          email,
          type,
  ...
  const handleLogin = (event) => {
    ...
    const user = serverData.data.find(
      (baznas_userData) => baznas_userData.email === username && baznas_userData.password === password
    );
    ...
    if (user) {
      ...
      localStorage.setItem("baznas_user", user.name);
      localStorage.setItem("baznas_id", user._id);
      localStorage.setItem('baznas_userData', JSON.stringify(filteredData));
      ...
      <div className="sticky top-0 w-full right-0 lg:px-10 px-4 py-2 cursor-pointer bg-white shadow z-[1000]">
          <div className="flex justify-between items-center">
              <div className="flex justify-center gap-4 items-center">
                  <img className="rounded-full h-16" src="https://upload.wikimedia.org/wikipedia/commons/e/e6/Logo_BAZNAS_RI-Hijau-01.png" />
